...
function get_favourite_info($book_id,$user_id)
{
  global $mysqli;

  $sql="SELECT * FROM tbl_favourite WHERE `book_id`='$book_id' AND `user_id`='$user_id'";
  $res=mysqli_query($mysqli,$sql);
...
function get_contiune_info($book_id,$user_id)
{
  global $mysqli;

  $sql="SELECT * FROM tbl_user_continue WHERE `book_id`='$book_id' AND `user_id`='$user_id'";
  $res=mysqli_query($mysqli,$sql);
 ...
 function checkSignSalt($data_info){
        ...
        if($data_arr['package_name']==PACKAGE_NAME){
            if($data_arr['sign'] == '' && $data_arr['salt'] == '' ){
                ...
                $set = array("status" =>1,"message" => "","success" => -1, "msg" => "Invalid sign salt.");
                ...
                }else if(CountRow("tbl_users","id=".$data_arr['user_id']." AND status = 0")!=0){
                              $set['status']="1";
                              $set['success']="0";
                              $set['message']="";
                              $set['msg']='Your account is disable.';
                              header('Content-Type: application/json; charset=utf-8');
                              echo $val= str_replace('\\/', '/', json_encode($set,JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
                   exit();
                ...
                if((isset($data_arr['user_id']) || $data_arr['user_id']!='') AND $data_arr['user_id']!=0){

                  global $mysqli;

                  $user_id=$data_arr['user_id'];

                 $sql="SELECT * FROM tbl_users WHERE tbl_users.`id`='$user_id' AND tbl_users.`status`=1";
                  $res=mysqli_query($mysqli, $sql);
                  ...
                  if(mysqli_num_rows($res) > 0){
                      ...
                      $set = array("status" =>2,"message" => "Your account is deactived or deleted by admin !");   
                      header( 'Content-Type: application/json; charset=utf-8' );
                      echo $val= str_replace('\\/', '/', json_encode($set,JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
                      exit(); 
                  }
             ...
             //$data['data'] = array("success" => -1, "msg" => "Invalid sign salt.");
            $set = array("status" => "-1", "message" => "Invalid package name");   
            header( 'Content-Type: application/json; charset=utf-8' );
            echo $val= str_replace('\\/', '/', json_encode($set,JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
            exit();  
        }
